...
lua_State *stateL;
struct speedtestprogress prog[4];
...
void init_curl(CURLM *multi_handle, const char *link, int id, double testTime, int upload, lua_State *L)
{
  CURL *curl = NULL;
  ...
  if(curl) {
    ...
    prog[id].lastruntime = 0;
    prog[id].curl = curl;
    prog[id].L = L;
    prog[id].testtime = testTime;
    printf("%s %d\n",link, prog -> id);
    prog[id].id = id;
    printf("%s %d\n",link, prog -> id);
...
int test_internet_speed(const char *link, double testTime, int upload, lua_State *L)
{
  ...
  printf("Before adding all of the easycurls \n");
  for (int i = 0; i < 4; ++i)
  {
    printf("Adding one..\n");
    init_curl(multi_handle, link, i, testTime, upload, L);
  }
  printf("Afters adding all of the easycurls \n");

  /* we start some action by calling perform right away */
  curl_multi_perform(multi_handle, &still_running);
  while(still_running) {
    printf("%d", still_running);
